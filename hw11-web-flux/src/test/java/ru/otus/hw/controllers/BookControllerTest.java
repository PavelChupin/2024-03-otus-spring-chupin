...
//
//        mockMvc.perform(get(String.format("/api/v1/book/%s", bookDto.getId())))
...
//
//        mockMvc.perform(put(String.format("/api/v1/book/%s", bookUpdateDtoByBook.getId()))
...
//        mockMvc.perform(delete(String.format("/api/v1/book/%s", books.get(2).getId())))
//                .andExpect(status().isOk());
//    }
//
...
//        doThrow(RuntimeException.class).when(bookService).deleteById(anyString());
//
//        mockMvc.perform(delete(String.format("/api/v1/book/%s", books.get(2).getId())))
//                .andExpect(status().isInternalServerError());
//    }
